...
def save_segmentation_nifti_from_softmax(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
    ...
    #     np.savez_compressed(resampled_npz_fname, probabilities=seg_old_spacing.astype(np.float16)) # JJ: the same as nnUNet
    ...
    bbox = properties_dict.get('crop_bbox')
    if bbox is not None:
        seg_old_size = np.zeros([4, shape_original_before_cropping[0], shape_original_before_cropping[1], shape_original_before_cropping[2]]) # (a,b,z)->(4,a,b,c)
        seg_old_size[0] = 1 # bg:0
        for c in range(3):
        ...
        for ch in range(4):
            ...
            bbox[0][0]:bbox[0][1],
            bbox[1][0]:bbox[1][1],
            bbox[2][0]:bbox[2][1]] = seg_old_spacing[ch]
    else:
        seg_old_size = seg_old_spacing

    if resampled_npz_fname is not None:
        np.savez_compressed(resampled_npz_fname, probabilities=seg_old_size.astype(np.float16)) # JJ: the same as nnUNet
        # this is needed for ensembling if the nonlinearity is sigmoid
        if region_class_order is not None:
            properties_dict['regions_class_order'] = region_class_order
        save_pickle(properties_dict, resampled_npz_fname[:-4] + ".pkl")
    ...
    if seg_postprogess_fn is not None:
        ...
        seg_old_size_postprocessed = seg_old_size # JJ

    if region_class_order is None:
        seg_old_size_postprocessed = seg_old_size_postprocessed.argmax(0) # JJ
    else:
        seg_old_size_postprocessed_final = np.zeros(seg_old_size_postprocessed.shape[1:])
        for i, c in enumerate(region_class_order):
            seg_old_size_postprocessed_final[seg_old_size_postprocessed[i] > 0.5] = c
        seg_old_size_postprocessed = seg_old_size_postprocessed_final
